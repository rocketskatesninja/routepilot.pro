...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <x-stat-card 
            ...
            color="primary"
            icon="<path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M9 17v-2a4 4 0 014-4h2a4 4 0 014 4v2'></path>"/>
        <x-stat-card 
            title="This Month" 
        ...
        <x-stat-card 
            title="Admin/Tech Reports" 
            :value="$stats['admin_tech'] ?? 0" 
            color="info"
            ...
            <table class="table table-zebra w-full">
                ...
                <tbody>
                    ...
                    <tr>
                                    ...
                                    <div class="mask mask-squircle w-12 h-12">
                                        ...
                                        @else
                                            <div class="bg-primary text-primary-content rounded-lg flex items-center justify-center">
                                                <span class="text-lg font-semibold">{{ $report->client ? substr($report->client->first_name, 0, 1) . substr($report->client->last_name, 0, 1) : '?' }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                <div>
                                    <div class="font-bold text-base-content">{{ $report->client->full_name ?? '-' }}</div>
                        ...
                        <td>
                            <div class="text-sm">
                                <div class="text-base-content">{{ $report->location->name ?? '-' }}</div>
                                <div class="text-base-content/70">{{ $report->location->city ?? '' }}, {{ $report->location->state ?? '' }}</div>
                            </div>
                        </td>
                        @if(auth()->user()->role === 'admin')
                        <td>
                            <div class="flex items-center space-x-3">
                                <div class="avatar">
                                    <div class="mask mask-squircle w-10 h-10">
                                        ...
                                        @else
                                            <div class="bg-secondary text-secondary-content rounded-lg flex items-center justify-center w-10 h-10">
                                                <span class="text-lg font-semibold">{{ $report->technician ? substr($report->technician->first_name, 0, 1) . substr($report->technician->last_name, 0, 1) : '?' }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                <div>
                                    <div class="font-bold text-base-content">{{ $report->technician->full_name ?? '-' }}</div>
